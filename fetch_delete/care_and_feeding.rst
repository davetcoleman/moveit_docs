...
the public ros package server and the MoveIt! package server.

::

    >$ sudo sh -c 'echo "deb http://packages.moveit.com/ros/ubuntu trusty main" > /etc/apt/sources.list.d/fetch-latest.list'

    >$ sudo sh -c 'echo "deb http://packages.moveit.com/ros/ubuntu trusty main" > /etc/apt/sources.list.d/ros-latest.list'
